...
// but for macOS. Used by Dockerfile.macos to obtain the macOS SDK when
// cross-compiling JavaScriptCore for macOS on a Linux host.
//
...
//   regenerate: bun build --minify-syntax --target=bun xmac.ts --outfile=<WebKit>/macos-cross/xmac.mjs
//               (then re-add this header)
//
// The tool only talks to swscan.apple.com / swdist.apple.com / swcdn.apple.com
// and requires `xz` on PATH. Run `bun macos-cross/xmac.mjs --help` for usage.
//
